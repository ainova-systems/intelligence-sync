...
**Migration engine + bash↔skill contract:** `lib/layout.sh` (`detect_layout` → `LS_*`) and `lib/migrations.sh` (`MIGRATIONS=(...)` ordered registry + `run_migrations` dispatcher + version-named `migrate_to_<v>` + `check_version_compat` + `is_status`). Bash is the **deterministic, fail-closed core — it never guesses**: each `migrate_to_*` is idempotent (silent no-op once applied), runs in version order, uses **copy → verify sentinel → only then delete legacy**, and any unresolvable state emits a machine-readable `IS_STATUS=<code>` line + stable exit code (`IS_RC_*`: ok/migrated 0, error 1, config-missing 2, ambiguous 3, ahead-of-engine 4, aborted-incomplete 5) and stops. `check_version_compat` refuses to operate when the stamp is newer than `scripts/VERSION` (`ahead-of-engine`). The **`intelligence-update` skill is the intelligent layer**: triggered by "Update intelligence-sync", it detects state, bootstraps the engine from upstream, runs bash, branches on `IS_STATUS`, resolves ambiguity (asking the user), and verifies. `migrate_to_0_3_1` relocates pre-0.3.1 flat → `sync/` (meta-skills moved, never duplicated; one idempotent `config.yaml` `sources.skills` line). `sync.sh` runs it offline + re-execs from the module so it never deletes its own running dir; `update.sh` runs it with the fresh upstream clone as authoritative source. Version stamped at `<umbrella>/sync/.intelligence-sync-version`. Contract is documented in `docs/CONVENTIONS.md`; callers capture the rc via `cmd || rc=$?` (never `if ! cmd; then exit $?` — that loses the code).

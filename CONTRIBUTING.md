...
3. Make your changes
4. Test with at least one adapter (`bash intelligence/scripts/sync.sh claude`) — `sync.sh` runs `lint_frontmatter` over every source file before adapters fire, so unquoted YAML colons and other parse hazards surface as warnings on stderr.
...
Downstream projects pick up engine changes via `intelligence/scripts/update.sh` — point its `REPO_URL` at your fork to dry-run a release before merging:

```bash
REPO_URL=https://github.com/<you>/intelligence-sync.git \
  bash intelligence/scripts/update.sh --yes
```

`update.sh` clones into `mktemp -d`, replaces `intelligence/scripts/` and `intelligence/INIT.md`, and never touches project content (`config.yaml`, `rules/`, `agents/`, `skills/`).
...
- Use helpers from `intelligence/scripts/lib/common.sh` — never duplicate frontmatter parsing, model resolution, or YAML reading logic.

...
- `.gitignore` entries for `.claude`, `.cursor`, `CLAUDE.md`
...
3. **Targets to enable**:
   ...
   - Present as: "I detected [existing configs]. I will enable: `agents` + `<detected IDE>`. Other adapters (Claude, Cursor, Copilot, Codex) can be added any time via `/intelligence-install-adapter`."
...
targets:
  ...
  # Add more adapters as needed (cursor, copilot, codex)
...
- Only `.claude/settings.json` and `.cursor/settings.json` are tracked — they hold project-shared IDE settings (allowed bash commands, tool permissions). Everything else under those directories is either generated by sync or per-user state.
...
4. Report to user:
   ...
   - **Available but not enabled**: list remaining adapters (e.g., "codex — enable via `/intelligence-install-adapter codex`")

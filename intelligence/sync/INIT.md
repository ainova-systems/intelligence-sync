...
- `.gitignore` entries for `.claude`, `.cursor`, `.pi`, `CLAUDE.md`
...
3. **Targets to enable**:
   ...
   - Present as: "I detected [existing configs]. I will enable: `agents` + `<detected IDE>`. Other adapters (Claude, Cursor, Copilot, Codex, Pi) can be added any time via `/intelligence-install-adapter`."
...
targets:
  ...
  # Optional adapters: cursor, copilot, codex, pi
...
4. Report to user:
   ...
   - **Available but not enabled**: list remaining adapters (e.g., "codex, pi — enable via `/intelligence-install-adapter <name>`")

...
- **`targets.*.output` can no longer escape the repository, and `agents` is no longer exempt from the check.** `validate_output_path` compared the raw config string, so a `../` walked straight past every guard, and the `agents` adapter skipped validation entirely — a config line could point adapter cleanup (`rm -rf`) at a directory outside the repo, or aim the `agents` writer at an arbitrary file. Output paths are now lexically canonicalized (new `normalize_path` helper: `..`, `.` and `//` collapsed without touching the filesystem, since the path may not exist yet), required to resolve inside the repository, checked against symlinks that lead out of it, and validated for **every** adapter including `agents`. Configured source directories are canonicalized before the overlap check too. Not exploitable with a sane config — but a tool people vendor into their repos should not have a config-file-to-arbitrary-write path.
